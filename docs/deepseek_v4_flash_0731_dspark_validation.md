...
It supports pure TP8+DSpark and single-service DP+EP+DSpark. The public
configuration follows the standard vLLM `EngineArgs`/CLI path and the
...
checkpoint. Neither command needs `--enforce-eager`: TP8 retains vLLM graph
...
Both services use the same OpenAI-compatible client request:
...
records. EvalScope 1.11 leaves an opening Markdown fence in three otherwise
...
TP8 and DP8+EP8 each produced 32 predictions and 32 reviews. TP8's raw
EvalScope score is 30/32 (`0.9375`), while DP8+EP8's raw score is 29/32
(`0.9062`); the failures are otherwise valid completions wrapped in incomplete
Markdown fences. The source-controlled normalized re-execution passes 32/32
(`1.0`) for both topologies. The single-service DP path synchronizes
