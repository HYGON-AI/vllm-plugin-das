...
def test_vl_bias_routing_never_calls_lightop(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _install_lightop_moe(
        monkeypatch,
        moe_fused_gate_sqrtsoftplus=lambda *args, **kwargs: pytest.fail(
            "VL bias routing must remain on the official vLLM operator"
        ),
    )
    monkeypatch.setattr(henvs, "VLLM_HCU_USE_CUSTOM_OPS", True)
    monkeypatch.setattr(
    ...
    assert _call_patched(
        ...
        correction_bias=_CudaTensorMetadata((256,), torch.float32),
        input_tokens=torch.tensor([1, 2], dtype=torch.int64),
        bias_vl=torch.empty((256,), dtype=torch.float32),
        image_sentinel_lo=10,
    ) is official_result
