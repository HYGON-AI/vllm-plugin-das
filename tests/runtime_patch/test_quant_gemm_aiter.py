...
def test_aiter_asm_vllm_silu_context_changes_only_scoped_activation(
    monkeypatch: pytest.MonkeyPatch,
):
    ...
    asm_module = _module(
        "aiter.fused_moe_asm_wna16",
        _apply_activation=original_activation,
    )
    monkeypatch.setitem(
        sys.modules,
        "aiter.fused_moe_asm_wna16",
    ...
    def vllm_silu_and_mul(output, input):
        del input
        output.fill_(2)
    ...
    with aiter_runtime.aiter_asm_vllm_silu_context(enabled=True):
        asm_module._apply_activation(**kwargs)
...
def test_aiter_asm_vllm_silu_context_preserves_extended_activation_abi(
    monkeypatch: pytest.MonkeyPatch,
):
    ...
    def original_activation(
        ...
        original_calls.append(future_option)
        activated_out.fill_(3)
    ...
    monkeypatch.setattr(
        aiter_runtime,
        "_vllm_silu_and_mul",
        lambda output, input: output.fill_(2),
    )
    output = torch.empty((2, 4))
    input = torch.empty((2, 8))
    ...
    assert original_calls == ["preserved"]
    ...
    torch.testing.assert_close(output, torch.full_like(output, 3))
    assert original_calls == ["preserved", "preserved-silu"]


def test_aiter_asm_vllm_silu_context_rejects_incompatible_activation_abi(
    monkeypatch: pytest.MonkeyPatch,
):
    def incompatible_activation(activation, is_gated, activated_out):
        del activation, is_gated, activated_out

    asm_module = _module(
        "aiter.fused_moe_asm_wna16",
        _apply_activation=incompatible_activation,
    )
    monkeypatch.setitem(
        sys.modules,
        "aiter.fused_moe_asm_wna16",
    ...
    with pytest.raises(
        aiter_runtime.HcuAiterRuntimeError,
        match="incompatible ABI",
    ):
        with aiter_runtime.aiter_asm_vllm_silu_context(enabled=True):
            pass

    assert asm_module._apply_activation is incompatible_activation


def test_aiter_asm_vllm_silu_context_is_thread_local(
    monkeypatch: pytest.MonkeyPatch,
):
    def original_activation(
        *,
    ...
    enabled_entered = threading.Event()
    disabled_finished = threading.Event()
    results: dict[bool, torch.Tensor] = {}

    def run_enabled():
        output = torch.empty((2, 4))
        with aiter_runtime.aiter_asm_vllm_silu_context(enabled=True):
            enabled_entered.set()
            assert disabled_finished.wait(timeout=5)
            asm_module._apply_activation(
                activation="silu",
                is_gated=True,
                activated_out=output,
    ...
    def run_disabled():
        assert enabled_entered.wait(timeout=5)
        output = torch.empty((2, 4))
        with aiter_runtime.aiter_asm_vllm_silu_context(enabled=False):
            asm_module._apply_activation(
                activation="silu",
                is_gated=True,
                activated_out=output,
        ...
        results[False] = output
        disabled_finished.set()

    enabled_thread = threading.Thread(target=run_enabled)
    disabled_thread = threading.Thread(target=run_disabled)
    enabled_thread.start()
    disabled_thread.start()
    enabled_thread.join(timeout=10)
    disabled_thread.join(timeout=10)

    assert not enabled_thread.is_alive()
    assert not disabled_thread.is_alive()
    torch.testing.assert_close(results[True], torch.full((2, 4), 2.0))
    torch.testing.assert_close(results[False], torch.ones((2, 4)))


@pytest.mark.parametrize(
    ("use_fp8", "use_int8", "solution_type", "activation", "expected"),
    [
        (False, True, "asm", "silu", 2),
        (True, False, "asm", "silu", 1),
        (False, True, "triton", "silu", 1),
        (False, True, "asm", "gelu", 1),
    ],
)
def test_quantized_aiter_activation_alignment_is_int8_asm_silu_only(
    monkeypatch: pytest.MonkeyPatch,
    use_fp8: bool,
    use_int8: bool,
    solution_type: str,
    ...
    def aiter_moe(**kwargs):
        output = torch.empty_like(kwargs["hidden_states"])
        asm_module._apply_activation(
            activation=kwargs["activation"],
            is_gated=True,
            activated_out=output,
            ffn1_out_2d=torch.empty(
                output.shape[0],
                output.shape[1] * 2,
            ),
            gemm1_alpha=None,
