...
def test_aiter_and_triton_expert_capability_contract(
    ...
    fp8_quantized_result = aiter_module.rocm_aiter_fused_experts(
        hidden_states,
        w1,
        w2,
        topk_weights,
    ...
    assert fp8_quantized_result == "public-aiter-quantized"
    assert quantized_calls[1]["quant_config"] is fp8_quant_config
    assert quantized_calls[1]["activation"] is MoEActivation.SILU
    assert AiterExperts.is_supported_config(
        AiterExperts,
