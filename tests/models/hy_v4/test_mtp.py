...
def test_mtp_target_and_draft_share_all_topk_storage_through_pinned_loader(pp_size, monkeypatch):
    ...
    loaded = utils.load_eagle_model(target, SimpleNamespace(speculative_config=SimpleNamespace(draft_model_config=object())))
...
def test_mtp_shared_head_retains_storage_dtype_with_fp32_logits(monkeypatch):
    mtp = _mtp()
    ...
    patch_logits_processor_head_dtype.apply_to_module(logits_processor)
    monkeypatch.setattr(vocab_parallel_embedding, "get_tensor_model_parallel_rank", lambda: 0)
    monkeypatch.setattr(vocab_parallel_embedding, "get_tensor_model_parallel_world_size", lambda: 1)
    config = HYV4Config(vocab_size=64, hidden_size=16)
    config.model_type = "hy_v4_mtp"
    current = SimpleNamespace(model_config=SimpleNamespace(hf_config=config, head_dtype=torch.float32))
    with set_current_vllm_config(current):
        head = mtp.HYV4SharedHead(config).to(torch.bfloat16)
        processor = logits_processor.LogitsProcessor(64)
    processor._gather_logits = lambda logits: logits
    with torch.no_grad():
        head.head.weight.fill_(0.5)
    hidden = torch.full((2, 16), 0.5, dtype=torch.bfloat16)
    logits = processor(head.head, head(hidden))
    assert logits.dtype == torch.float32
    assert head.head.weight.dtype == torch.bfloat16
