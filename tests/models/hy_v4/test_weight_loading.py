...
def test_v0251_logits_processor_accumulates_unquantized_hy_v4_head_in_fp32(
    ...
    current = SimpleNamespace(
        model_config=SimpleNamespace(
            head_dtype=torch.float32, hf_config=SimpleNamespace(model_type="hy_v4")),
    )
...
def test_head_dtype_adapter_preserves_unrelated_model_logits_dtype(monkeypatch):
    patch_logits_processor_head_dtype.apply_to_module(logits_module)
    monkeypatch.setattr(vocab_module, "get_tensor_model_parallel_rank", lambda: 0)
    monkeypatch.setattr(vocab_module, "get_tensor_model_parallel_world_size", lambda: 1)
    # The current ROCm GEMM dispatcher has no CPU kernel. Keep the real
    # embedding method and replace only its accelerator GEMM boundary.
    monkeypatch.setattr(
        vocab_module,
        "dispatch_unquantized_gemm",
        lambda: lambda layer, x, weight, bias: torch.nn.functional.linear(x, weight, bias),
    )
    current = SimpleNamespace(model_config=SimpleNamespace(
        head_dtype=torch.float32, hf_config=SimpleNamespace(model_type="unrelated")))
    with set_current_vllm_config(current):
        processor = LogitsProcessor(vocab_size=64)
    processor._gather_logits = lambda logits: logits
    head = ParallelLMHead(64, 16, params_dtype=torch.bfloat16)
    ...
    actual = processor(head, hidden)
    assert actual.dtype == torch.bfloat16
    torch.testing.assert_close(actual, torch.full((2, 64), 8.0, dtype=torch.bfloat16))
