...
def test_named_tool_choice_constrains_hyv4_structural_tag() -> None:
    parser = HYV4ToolParser(FakeTokenizer(_tool_vocabulary()))
    request = ChatCompletionRequest(
        model="hy4",
        messages=[{"role": "user", "content": "Use date only"}],
        tools=[
            {
                "type": "function",
                "function": {
        ...
        tool_choice={"type": "function", "function": {"name": "date"}},
    )

    structural_tag = parser.get_structural_tag(request)
    assert structural_tag is not None
    grammar = Grammar.from_structural_tag(structural_tag)
    assert _is_grammar_accept_string(grammar, _block(_call("date")))
    assert not _is_grammar_accept_string(grammar, _block(_call("weather")))
