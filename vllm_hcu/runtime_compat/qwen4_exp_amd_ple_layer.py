...
class Qwen4ExpNGramEmbedding(nn.Module):
    ...
    def _forward_impl(
        ...
        if (
            ...
            and num_reqs > self.padded_buffer.shape[0]
        ):
            raise ValueError(
    ...
    def forward(
        ...
        torch.ops.vllm.qwen4_exp_amd_ple_forward(
...
def qwen4_exp_amd_ple_forward(
    ...
    if not isinstance(layer, Qwen4ExpPLELayer):
        raise TypeError(f"{layer_name} is not a Qwen4Exp PLE owner")
    result = layer.ple_embedding._forward_impl(
        input_ids, query_start_loc, ngram_context
    )
    output.copy_(result)


def qwen4_exp_amd_ple_forward_fake(
    input_ids: torch.Tensor,
    query_start_loc: torch.Tensor,
    ngram_context: torch.Tensor,
    output: torch.Tensor,
    ...
    del input_ids, query_start_loc, ngram_context, output, layer_name


def qwen4_exp_amd_mtp_hidden_copy(
    hidden_states: torch.Tensor,
    buffer: torch.Tensor,
) -> None:
...
def qwen4_exp_amd_mtp_hidden_copy_fake(
    hidden_states: torch.Tensor,
    buffer: torch.Tensor,
) -> None:
    del hidden_states, buffer
...
direct_register_custom_op(
    op_name="qwen4_exp_amd_ple_forward",
    op_func=qwen4_exp_amd_ple_forward,
    mutates_args=["output"],
    fake_impl=qwen4_exp_amd_ple_forward_fake,
)


direct_register_custom_op(
    op_name="qwen4_exp_amd_mtp_hidden_copy",
    op_func=qwen4_exp_amd_mtp_hidden_copy,
    mutates_args=["buffer"],
    fake_impl=qwen4_exp_amd_mtp_hidden_copy_fake,
)
