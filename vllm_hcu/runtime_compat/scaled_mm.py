...
def install_fp8_scaled_mm_compat(module: ModuleType | None = None) -> None:
    ...
    support_descriptor = vars(ChannelWiseTorchFP8ScaledMMLinearKernel).get(
        "is_supported"
    )
    ...
    original_is_supported = getattr(
        ChannelWiseTorchFP8ScaledMMLinearKernel,
        "is_supported",
        None,
    )

    def hcu_is_supported(cls, compute_capability=None):
        del cls, compute_capability
        return True, None

    hcu_is_supported._hcu_fp8_support_wrapper = True
    ...
    ChannelWiseTorchFP8ScaledMMLinearKernel.is_supported = classmethod(
        hcu_is_supported
    )
    ...
    ChannelWiseTorchFP8ScaledMMLinearKernel._hcu_original_is_supported = (
        original_is_supported
    )
