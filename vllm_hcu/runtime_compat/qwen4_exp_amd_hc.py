...
def _hc_gate_mix(x: torch.Tensor, gate: torch.Tensor, hc_count: int) -> torch.Tensor:
    ...
    if x.shape != gate.shape:
        if (
            ...
            # Speculative profile buffers reserve one extra token slot.  The
            # gate and residual can therefore differ by padded rows at a
            # compiled graph boundary; only their common prefix is valid.
            N = min(x.shape[0], gate.shape[0])
            x = x[:N]
            gate = gate[:N]
        else:
            raise ValueError(
                "HC gate mix inputs have incompatible shapes: "
                f"{tuple(x.shape)} and {tuple(gate.shape)}"
            )
    else:
        N = x.shape[0]
...
def _hc_combine(
    ...
    assert DIM % hc_count == 0
    hc_dim = DIM // hc_count
    if block_output.shape != (N, hc_dim):
        if block_output.ndim == 2 and block_output.shape[1] == hc_dim:
            if block_output.shape[0] < N:
                raise ValueError(
                    "HC block output has too few rows: "
                    f"{tuple(block_output.shape)} vs {(N, hc_dim)}"
                )
            block_output = block_output[:N]
        elif block_output.numel() == N * hc_dim:
            block_output = block_output.contiguous().view(N, hc_dim)
        else:
            raise ValueError(
                "HC block output has incompatible size: "
                f"{tuple(block_output.shape)} vs {(N, hc_dim)}"
            )
    if injection_logits.shape != (N, hc_count):
        if injection_logits.ndim == 2 and injection_logits.shape[1] == hc_count:
            if injection_logits.shape[0] < N:
                raise ValueError(
                    "HC injection logits have too few rows: "
                    f"{tuple(injection_logits.shape)} vs {(N, hc_count)}"
                )
            injection_logits = injection_logits[:N]
        elif injection_logits.numel() == N * hc_count:
            injection_logits = injection_logits.contiguous().view(N, hc_count)
        else:
            raise ValueError(
                "HC injection logits have incompatible size: "
                f"{tuple(injection_logits.shape)} vs {(N, hc_count)}"
            )
