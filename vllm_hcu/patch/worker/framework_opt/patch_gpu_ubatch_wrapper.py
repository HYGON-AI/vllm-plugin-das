...
"""Guard main's DBO SM control when vendor DeepGEMM lacks its SMS API."""
...
TARGETS = (
    ...
    f"{TARGET_MODULE}.create_sm_control_context",
)
...
def apply_to_module(module: ModuleType) -> bool:
    ...
    wrapped = ((ubatch, "create_sm_control_context", TARGETS[1], _WRAPPER),)
    if already_applied(ubatch, _MARKER, wrapped):
        return False

    original = require_callable(ubatch, "create_sm_control_context", TARGETS[1])
    require_exact_signature(original, TARGETS[1], positional=("parallel_config",))
    require_callable(ubatch, "deep_gemm_set_num_sms", TARGETS[0])

    @functools.wraps(original)
    def hcu_create_sm_control_context(parallel_config):
        from vllm_hcu.v1 import worker_framework_runtime

        if worker_framework_runtime.deep_gemm_has_sms_api(ubatch):
            return original(parallel_config)
        return worker_framework_runtime.create_sm_control_context_without_compute(
            ubatch, parallel_config
        )

    setattr(hcu_create_sm_control_context, _WRAPPER, True)
    setattr(ubatch, "_vllm_hcu_original_create_sm_control_context", original)
    setattr(ubatch, "create_sm_control_context", hcu_create_sm_control_context)
