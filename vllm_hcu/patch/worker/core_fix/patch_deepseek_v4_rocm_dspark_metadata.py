...
_INIT_WRAPPER_MARKER = "_vllm_hcu_dspark_ragged_builder_init_wrapper"
_COPY_WRAPPER_MARKER = "_vllm_hcu_dspark_ragged_copy_wrapper"
...
def apply_to_module(module: ModuleType) -> bool:
    ...
    if getattr(rocm, _MARKER, False):
        current_init = vars(builder_cls).get("__init__")
        current_copy = vars(rocm).get("_copy_ragged_to_graph_buffers")
        if not getattr(current_init, _INIT_WRAPPER_MARKER, False) or not getattr(
            current_copy, _COPY_WRAPPER_MARKER, False
        ):
    ...
    setattr(hcu_dspark_builder_init, _INIT_WRAPPER_MARKER, True)
    setattr(hcu_copy_ragged_to_graph_buffers, _COPY_WRAPPER_MARKER, True)
    setattr(builder_cls, "_vllm_hcu_original_dspark_builder_init", original_init)
    setattr(builder_cls, "__init__", hcu_dspark_builder_init)
    setattr(rocm, "_vllm_hcu_original_copy_ragged_to_graph_buffers", original_copy)
