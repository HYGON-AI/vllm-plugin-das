...
class HYV4FlashMLASparseImpl(FlashMLASparseImpl):
    ...
    def _bf16_flash_mla_kernel_with_lse(
        ...
        query_heads = q.shape[1] if actual_num_heads is None else actual_num_heads
        if q.shape[1] < query_heads:
            raise ValueError("actual_num_heads exceeds query head count")
        ...
        attn_sink = self._sinks_for_query(
            q[:, :query_heads, :], head_dim=1, kernel_heads=kernel_heads
        )

        # NOTE(Chen): kernel requires num_local_head to be a multiple of
        # 64 on hopper and 128 on blackwell
        if q.shape[1] < kernel_heads:
            logger.warning_once(
