...
class Indexer(nn.Module):
    def __init__(
        ...
        self.k_cache = DeepseekV32IndexerCache(
            head_dim=self.head_dim + self.head_dim // self.quant_block_size * 4 if on_gfx938() else self.head_dim,
            dtype=torch.uint8 if on_gfx938() else torch.bfloat16,
...
class DeepseekV2MLAAttention(nn.Module):
    ...
    def __init__(
        ...
        self.is_v32 = hasattr(config, "index_topk") and not henvs.VLLM_HCU_DISABLE_DSA
...
class DeepseekV2Model(nn.Module):
    ...
    def forward(
        ...
        enable_lightly_cp = get_forward_context().enable_lightly_cp
        if enable_lightly_cp:
            hidden_states, positions, residual, _ = lightly_cp_inputs_splitting(
                hidden_states,
                positions,
                residual,
                None,
