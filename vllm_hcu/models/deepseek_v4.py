    ...
    def _init_fused_moe_experts(
        ...
        if enable_ep:
            ...
            self.experts_end_idx = (
                self.experts_start_idx + self.n_local_physical_experts
            )
...
class DeepseekV4Attention(nn.Module):
    def __init__(
        ...
        # Keep attention sink aligned with the real local head count.
        padded_heads = self.n_local_heads
        # Legacy padded-head sink path, disabled because FlashMLA now supports
...
class DeepseekV4Model(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # kv_score). fused_wqa_wkv stays on the default stream. Individual
        # overlap sites are threshold/graph gated where profiling shows eager
        # contention; this env switch provides a coarse rollback knob.
        aux_stream_list = (
...
class DeepseekV4MixtureOfExperts(MixtureOfExperts):
    ...
    def extract_moe_parameters(
        self, example_moe: "DeepseekV4MoE | None"
    ) -> None:
...
class DeepseekV4ForCausalLM(
    nn.Module, SupportsPP, DeepseekV4MixtureOfExperts
):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Collect MoE layers for EPLB / MixtureOfExperts.
        self.moe_mlp_layers = []
        self.moe_layers = []
        self.expert_weights = []
