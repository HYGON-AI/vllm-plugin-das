...
def _apply_kimi_mla_output_gate(
    attn_out: torch.Tensor,
    hidden_states: torch.Tensor,
    g_proj: nn.Module,
) -> torch.Tensor:
    """Apply Kimi's sigmoid gate before the MLA output projection."""
    return attn_out * g_proj(hidden_states)[0].sigmoid()


def get_spec_layer_idx_from_weight_name(config: Any, weight_name: str) -> int | None:
    """Compatibility helper for vLLM wheels predating the MTP utility."""

    num_nextn = getattr(config, "num_nextn_predict_layers", 0)
    if not num_nextn:
        return None
    base_layer = config.num_hidden_layers
    for index in range(num_nextn):
        layer_index = base_layer + index
...
def _uses_native_k3_mxfp4_partition(
    config: KimiLinearConfig, quant_config: QuantizationConfig | None
) -> bool:
    """K3's SiTU MXFP4 experts support their native TP shard width."""
    return (
        quant_config is not None
        and quant_config.get_name() == "mxfp4"
        and config.hidden_act == "situ"
        and config.activation_situ_beta is not None
        and config.activation_situ_linear_beta is not None
    )


def _padded_k3_moe_intermediate_size(
    config: KimiLinearConfig,
    quant_config: QuantizationConfig | None,
    tp_size: int,
) -> int:
    """Apply legacy padding unless the native K3 MXFP4 or W4A8 path is selected."""
    moe_intermediate_size = config.moe_intermediate_size
    assert moe_intermediate_size is not None
    if _uses_native_k3_mxfp4_partition(config, quant_config):
        return moe_intermediate_size
    # The W4A8 per-expert Triton kernel only requires %32 alignment, and the
    # native 192-wide TP16 partition is already %32-aligned. The legacy
    # 256-wide per-partition padding would inflate w13/w2 enough to OOM the
    # full-model TP16 run (observed ~124 GiB/rank allocation vs 136 GiB cap).
    if quant_config is not None and quant_config.get_name() == "kimi_k3_w4a8":
        return moe_intermediate_size

    min_per_partition = getattr(config, "min_moe_intermediate_per_partition", 256)
    if tp_size > 1 and moe_intermediate_size // tp_size < min_per_partition:
        return min_per_partition * tp_size
    return moe_intermediate_size
...
class KimiRoutedOutputTransform(nn.Module):
    def __init__(
        ...
        # MoERunner uses this Kimi-specific capability to concatenate the
        # latent routed and shared-expert partials before one TP all-reduce.
        self._hcu_fuse_shared_and_routed_tp_all_reduce = True
...
class _KimiMLAOutputGateCompat(MultiHeadLatentAttentionWrapper):
    """Backport Kimi's MLA output gate to vLLM wheels without ``g_proj``."""
    ...
    def forward(
        ...
        if g_proj is not None:
            attn_out = _apply_kimi_mla_output_gate(attn_out, hidden_states, g_proj)
...
class KimiMoE(nn.Module):
    def __init__(
        ...
        self.padded_moe_intermediate_size = _padded_k3_moe_intermediate_size(
            config, quant_config, self.tp_size
        )
        ...
        if self.padded_moe_intermediate_size != moe_intermediate_size:
            # MoERunner keeps expert weights on its routed_experts submodule;
            # legacy runners expose them directly. Probe both names so the
            # padded-region zeroing works regardless of runner layout.
            experts = getattr(self.experts, "routed_experts", self.experts)
            w13_weight = getattr(experts, "w13_weight", None)
            if w13_weight is None:
                w13_weight = getattr(experts, "w13_weight_packed", None)
            w2_weight = getattr(experts, "w2_weight", None)
            if w2_weight is None:
                w2_weight = getattr(experts, "w2_weight_packed", None)
...
class KimiMLAAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        has_native_output_gate = "g_proj" in inspect.signature(MLAModules).parameters
        if has_native_output_gate:
            mla_kwargs["g_proj"] = getattr(self, "g_proj", None)
        mla_modules = MLAModules(**mla_kwargs)
        mla_wrapper_cls = MultiHeadLatentAttentionWrapper
        if self.use_output_gate and not has_native_output_gate:
            mla_wrapper_cls = _KimiMLAOutputGateCompat
        self.mla_attn = mla_wrapper_cls(
            self.hidden_size,
        ...
        if self.use_output_gate and not has_native_output_gate:
            self.mla_attn.g_proj = self.g_proj
        ...
        for args in weights:
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                for (
                    ...
                    # The generic mapping replaces the trailing ``w1.`` /
                    # ``w2.`` prefix. For slimquant channel scales that would
                    # produce ``w13_weightweight_scale``; normalize it to the
                    # registered fused scale parameter name.
                    name_mapped = name_mapped.replace(
                        "weightweight_scale", "weight_scale"
                    )
                    # Kimi slimquant registers routed experts directly under
                    # ``experts``. The generic mapping keeps the older
                    # ``experts.routed_experts`` convention for other models.
                    if name_mapped not in params_dict:
                        name_mapped = name_mapped.replace(
                    ...
                    if is_pp_missing_parameter(name, self):
                        continue
                    # Smoke configurations may truncate num_hidden_layers,
                    # while the streaming checkpoint iterator still yields
                    # tensors from subsequent layers. They have no matching
                    # registered parameter and must be skipped.
