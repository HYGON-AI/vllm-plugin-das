...
class DeepSeekV4MultiTokenPredictor(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # DeepseekV4Model. The env switch provides a coarse rollback knob;
        # overlap callers still decide whether eager or graph-captured
        # parallelism is profitable.
        aux_stream_list = (
            [torch.cuda.Stream() for _ in range(3)]
            if (
                henvs.VLLM_HCU_USE_CUSTOM_OPS
                and henvs.VLLM_HCU_ENABLE_DEEPSEEK_V4_MULTI_STREAM
            )
            else None
        )
...
class DeepSeekV4MTP(nn.Module):
    ...
    def set_moe_parameters(self) -> None:
        ...
        for layer in self.model.layers.values():
            ...
            if hasattr(mtp_block, "ffn") and hasattr(mtp_block.ffn, "experts"):
                example_moe = mtp_block.ffn
                self.moe_mlp_layers.append(mtp_block.ffn)
                self.moe_layers.append(mtp_block.ffn.experts)
        ...
        self.num_logical_experts = getattr(example_moe, "n_logical_experts", None)
        self.num_physical_experts = getattr(example_moe, "n_physical_experts", None)
        self.num_local_physical_experts = getattr(
            example_moe, "n_local_physical_experts", None
        )
        self.num_routed_experts = getattr(example_moe, "n_routed_experts", None)
        self.num_shared_experts = getattr(example_moe, "n_shared_experts", 0)
        self.num_redundant_experts = getattr(example_moe, "n_redundant_experts", None)
    ...
    def update_physical_experts_metadata(
        ...
        for moe in self.moe_mlp_layers:
            ...
            if hasattr(moe.experts, "update_expert_map"):
                ...
                if ".experts." in name:
                    ...
                    for mapping in expert_mapping:
                        ...
                        if name_mapped not in params_dict:
                            name_mapped = maybe_remap_compressed_tensors_scale_name(
                                name_mapped
                            )
