...
class CustomAllreduce {
 public:
  ...
  CustomAllreduce(Signal** signals, void* rank_data, size_t rank_data_sz,
    ...
    if (!fully_connected) {
      CUDACHECK(cudaMalloc((void**)&dev_curr_hdp_reg, world_size_ * sizeof(uint32_t*)));
      for (int i = 0; i < world_size_; ++i) {
        CUDACHECK(hipDeviceGetAttribute((int*)&dev_curr_hdp_reg[i], hipDeviceAttributeHdpMemFlushCntl, i));
      }
    }
