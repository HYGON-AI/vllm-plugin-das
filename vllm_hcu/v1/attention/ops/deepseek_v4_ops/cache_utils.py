...
def _enable_deepseek_v4_cache_window(num_blocks: int) -> bool:
    if (
        ...
        return False
    return num_blocks > _DEEPSEEK_V4_CACHE_WINDOW_BLOCK_THRESHOLD
...
@triton.jit
def _quantize_and_insert_k_window_kernel(
    # Input tensors
    k_ptr,  # [num_tokens, 512] bf16
    slot_mapping_ptr,  # [num_tokens] int64
    # Output tensor
    k_cache_ptr,  # [window_blocks, block_bytes] as uint8 (flattened view)
    # Dimensions
    num_tokens,
    input_dim: tl.constexpr,  # 512
    fp8_dim: tl.constexpr,  # 448
    ...
    token_data_size: tl.constexpr,  # 576 bytes per token data
    block_stride: tl.constexpr,  # total bytes per block (padded)
    window_start_block: tl.constexpr,
    ...
    block_idx = slot_idx // cache_block_size
    pos_in_block = slot_idx % cache_block_size
    if block_idx < window_start_block or block_idx >= window_start_block + window_blocks:
        return

    input_row_ptr = k_ptr + pid * input_dim
    local_block_idx = block_idx - window_start_block
    cache_block_ptr = k_cache_ptr + local_block_idx * block_stride
    token_data_ptr = cache_block_ptr + pos_in_block * token_data_size
    token_scale_ptr = (
        cache_block_ptr + cache_block_size * token_data_size + pos_in_block * scale_dim
    )
...
def quantize_and_insert_k_cache(
    ...
    grid = (num_tokens,)
    if _enable_deepseek_v4_cache_window(k_cache.shape[0]):
        window_blocks = max(1, _DEEPSEEK_V4_CACHE_WINDOW_BYTE_LIMIT // block_stride)
        for window_start_block in range(0, k_cache.shape[0], window_blocks):
            window_k_cache = k_cache[window_start_block:]
            _quantize_and_insert_k_window_kernel[grid](
                k,
                slot_mapping,
                window_k_cache,
                num_tokens,
...
@triton.jit
def _dequantize_and_gather_k_window_kernel(
    out_ptr,
    out_stride0,
    out_stride1,
    k_cache_ptr,
    ...
    for i in range(worker_id, gather_len, num_workers):
        ...
        in_window = (
            physical_block_idx >= window_start_block
            and physical_block_idx < window_start_block + window_blocks
        )
        if in_window:
            local_block_idx = physical_block_idx - window_start_block
            cache_block_ptr = k_cache_ptr + local_block_idx * block_stride
            token_data_ptr = cache_block_ptr + pos_in_block * token_data_size
            token_scale_ptr = (
                cache_block_ptr
...
def dequantize_and_gather_k_cache(
    ...
    if _enable_deepseek_v4_cache_window(k_cache.shape[0]):
        ...
        for window_start_block in range(0, k_cache.shape[0], window_blocks):
            window_k_cache = k_cache[window_start_block:]
            _dequantize_and_gather_k_window_kernel[(num_reqs, NUM_WORKERS)](
                out,
                out.stride(0),
                out.stride(1),
                window_k_cache,
...
def _compute_global_topk_indices_and_lens_kernel(
    ...
    for i in range(0, topk, TRITON_BLOCK_SIZE):
        ...
        is_valid = (local_idx >= 0) & (block_indices < block_table_stride)
