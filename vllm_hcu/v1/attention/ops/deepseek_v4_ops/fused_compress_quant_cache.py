...
# =============================================================================
# Windowed variants for large block-id cache layouts
# =============================================================================
@triton.jit
def _fused_kv_compress_norm_rope_insert_sparse_attn_window(
    # ── state cache (compressor internal state) ──
    state_cache_ptr,
    state_cache_stride0,
    state_cache_stride1,
    ...
    SCALE_DIM: tl.constexpr,  # 8 for DeepseekV4 (7 real + 1 pad)
    KV_BLOCK_STRIDE: tl.constexpr,
    STATE_WINDOW_START_BLOCK: tl.constexpr,
    STATE_WINDOW_BLOCKS: tl.constexpr,
    STATE_CORE_START_BLOCK: tl.constexpr,
    STATE_CORE_END_BLOCK: tl.constexpr,
    KV_WINDOW_START_BLOCK: tl.constexpr,
    KV_WINDOW_BLOCKS: tl.constexpr,
):
    """Fused compress → RMSNorm → FP8 quant (nope) → RoPE → bf16 store (rope).
    ...
    if slot_id < 0:
        return
    current_state_block = slot_id // block_size
    if current_state_block < STATE_CORE_START_BLOCK:
        return
    if current_state_block >= STATE_CORE_END_BLOCK:
        return
    ...
    state_window_end = STATE_WINDOW_START_BLOCK + STATE_WINDOW_BLOCKS
    out_of_state_window = (
        (block_numbers < STATE_WINDOW_START_BLOCK)
        | (block_numbers >= state_window_end)
    ) & mask_pos
    if tl.max(out_of_state_window.to(tl.int32), axis=0) != 0:
        return
    block_offsets = pos % block_size
    head_offset = (tokens >= COMPRESS_RATIO).to(tl.int32) * HEAD_SIZE
    ...
    row_base = (
        state_cache_ptr
        + local_block_numbers_i64 * state_cache_stride0
        + block_offsets * state_cache_stride1
        + head_offset
    )
    ...
    kv_block_idx = kv_slot_idx // kv_cache_block_size
    kv_pos_in_block = kv_slot_idx % kv_cache_block_size
    if kv_block_idx < KV_WINDOW_START_BLOCK:
        return
    if kv_block_idx >= KV_WINDOW_START_BLOCK + KV_WINDOW_BLOCKS:
        return

    local_kv_block_idx = kv_block_idx - KV_WINDOW_START_BLOCK
    cache_block_ptr = k_cache_ptr + local_kv_block_idx.to(tl.int64) * KV_BLOCK_STRIDE
    fp8_ptr = cache_block_ptr + kv_pos_in_block * TOKEN_STRIDE
    scale_ptr = (
        cache_block_ptr
        + kv_cache_block_size * TOKEN_STRIDE
...
# =============================================================================
@triton.jit
def _fused_kv_compress_norm_rope_insert_indexer_attn_window(
    # ── state cache (compressor internal state) ──
    state_cache_ptr,
    state_cache_stride0,
    state_cache_stride1,
...
# =============================================================================
@triton.jit
def _fused_kv_compress_norm_rope_insert_indexer_mxfp4_attn_window(
    # ── state cache (compressor internal state) ──
    state_cache_ptr,
    state_cache_stride0,
    state_cache_stride1,
