...
"""FP8-capable Triton sparse attention for the Qwen4Exp QSA main cache.

The official QSA kernel pair (``vllm.models.qwen4_exp.amd.ops.qsa``) is
BF16-only: its paged sparse GQA kernel loads K/V tiles and feeds them
straight into ``tl.dot`` with no per-tensor scale, so an FP8 main cache
cannot be consumed there.  This module mirrors that kernel with one
addition -- K/V tiles are dequantized in-register from FP8 to BF16 using the
layer's ``k_scale``/``v_scale`` before the dots -- which lets the QSA owner
keep an FP8 main KV cache.

Scaling algebra, since it justifies where the two multiplies sit:

* ``k_scale`` is a per-tensor scalar, so it multiplies *every* score in a row
  identically.  Folding it into the log2 softmax scale is therefore exact and
  avoids a second rounding of the key tile.
* ``v_scale`` is also per-tensor, so it is a uniform multiplier of the
  attention output.  It is applied once to the normalized result (for
  split-K, to each partial before the LSE merge) rather than per tile.

Both scales arrive as device tensors and are loaded inside the kernel, so no
host synchronization is introduced and the whole path stays CUDA-graph safe.
With the default unit scales the dequantization degenerates to a plain cast,
which is exactly the ``fp8_e5m2``/``fp8_e4m3`` contract for a checkpoint that
carries no calibrated KV scales (``kv_cache_scheme: null``).

The split-K merge kernel is reused verbatim from the official module: it
operates on FP32 partials and writes BF16 output, so it is already
dtype-agnostic and needs no FP8-specific variant.
"""
...
from vllm.models.qwen4_exp.amd.ops.qsa import (
    _qsa_merge_splitk_kernel as _merge_splitk_kernel,
)
from vllm.platforms import current_platform
from vllm.triton_utils import HAS_TRITON, tl, triton

__all__ = ["qsa_sparse_paged_attention_fp8"]
...
def _qsa_sparse_paged_gqa_splitk_fp8_kernel(
    ...
    # Per-tensor dequantization scales, loaded on device to avoid a host sync.
    ...
    # k_scale is uniform across keys, so it composes exactly with the log2
    # softmax scale instead of being applied to the key tile.
    ...
    for tile in range(split_tile_start, split_tile_end):
        ...
        # FP8 -> BF16.  k_scale is folded into the score scale below and
        # v_scale is deferred to the normalized output, so neither tile is
        # rescaled here.
...
def _resolve_scale(value: object, name: str, device: torch.device) -> torch.Tensor:
    """Validate a per-tensor scale and place it where the kernel can load it.

    ``Tensor.to`` returns the same object when the tensor is already on the
    requested device in the requested dtype, so the common path costs nothing.
    """

    if not isinstance(value, torch.Tensor):
        raise ValueError(f"QSA FP8 sparse attention requires a {name} tensor")
    if value.numel() != 1:
        raise ValueError(
            f"QSA FP8 sparse attention requires a per-tensor {name}"
        )
...
def qsa_sparse_paged_attention_fp8(
    ...
    """Sparse GQA over paged FP8 K/V caches.

    Signature-compatible with
    ``vllm.models.qwen4_exp.amd.ops.qsa.qsa_sparse_paged_attention`` except
    for the two trailing per-tensor scales.  ``k_cache``/``v_cache`` are the
    FP8 views of the paged cache; Q and out stay BF16.
    """
    ...
    if q.dtype != torch.bfloat16:
        raise ValueError("QSA FP8 sparse attention keeps Q in BF16")
    if k_cache.dtype != v_cache.dtype or k_cache.element_size() != 1:
        raise ValueError("QSA FP8 sparse attention requires a one-byte K/V cache")
    ...
    if out.dtype != torch.bfloat16:
        raise ValueError("QSA FP8 sparse attention writes BF16 output")
    assert out.stride(2) == 1
    if not q.shape[0]:
        return out

    if k_scale is None or v_scale is None:
        raise ValueError("QSA FP8 sparse attention requires K and V scales")
    k_scale = _resolve_scale(k_scale, "k_scale", q.device)
    v_scale = _resolve_scale(v_scale, "v_scale", q.device)
    ...
    # Mirrors the BF16 kernel's tuning, with split-K retained so long
    # selections keep the same parallelism profile.
    ...
    _merge_splitk_kernel[(q.shape[0], q.shape[1])](
        partial_output,
        partial_lse,
        out,
        out.stride(0),
