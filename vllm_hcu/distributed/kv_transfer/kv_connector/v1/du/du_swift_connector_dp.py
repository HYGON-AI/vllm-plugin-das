...
from vllm_hcu.distributed.kv_transfer.kv_connector.v1.du.du_swift_engine_dp import (
...
class DuSwiftConnectorDp(KVConnectorBase_V1):
    def __init__(
        ...
        self._local_rank = get_world_group().local_rank \
            if role == KVConnectorRole.WORKER else 0
        self._dp_rank = int(self.config.get_from_extra_config(
            "global_data_parallel_index",
            getattr(self.parallel_config, "data_parallel_index", 0),
        )) if role == KVConnectorRole.WORKER else 0
        self._pp_rank = get_pp_group().rank_in_group \
            if role == KVConnectorRole.WORKER else 0
        self._tp_rank = get_tp_group().rank_in_group \
            if role == KVConnectorRole.WORKER else 0
        self._dp_size = int(self.config.get_from_extra_config(
            "dp_size",
            getattr(self.parallel_config, "data_parallel_size", 1),
        )) if role == KVConnectorRole.WORKER else 0
        ...
        if role == KVConnectorRole.SCHEDULER :
            self.dp_rank = int(self.config.get_from_extra_config(
            "global_data_parallel_index",
            getattr(vllm_config.parallel_config, "data_parallel_index", dp_rank),
            ))
    ...
    def save_kv_layer(
        ...
        **kwargs: Any,
        ...
        def extract_kv_from_layer(
            ...
            if layer.shape[0] == 2:  # FlashAttention NV layout
                return layer[:, block_ids, ...]
            logger.error("🚧kv_cache not mla && gqa")
