...
class DuSwiftEngineDp:
    def __init__(self,
        ...
        if self.send_type == "GET":
            ...
            self.pending_queue: dict[str, list[SendQueueItem]] = defaultdict(list)
    ...
    def send_tensor(
        ...
        tensor_id: str,
        tensor: torch.Tensor,
        remote_address: typing.Optional[RemoteAddr] | None = None,
    ) -> bool:
        ...
        while True:
            ...
            if data["cmd"] == "NEW":
                ...
                with self.recv_store_cv:
                    ...
                    logger.info(
                        "[COMM_INIT] zmq=%s device=%s local_rank=%s dp=%s pp=%s tp=%s "
                        "pd_pair=%s world_size=%s rank=%s",
                        self.zmq_address,
                        self.device,
                        self.local_rank,
                        self.dp_rank,
    ...
    def get_finished(
        ...
        with self.recv_store_cv:
            for req in self.recv_request_id_to_tensor_ids:
                if req not in finished_req_ids:
                    continue
