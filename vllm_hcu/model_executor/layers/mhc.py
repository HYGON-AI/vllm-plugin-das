...
from deepgemm import tf32_hc_pernorm_gemm as tf32_hc_prenorm_gemm
...
def _normalize_deepgemm_mhc_split(split_k: int) -> int:
    for supported_split in reversed(_DEEPGEMM_MHC_SUPPORTED_SPLITS):
        if split_k >= supported_split:
            return supported_split
    return _DEEPGEMM_MHC_SUPPORTED_SPLITS[0]
...
def compute_num_split(block_k: int, k: int | None, grid_size: int) -> int:
    ...
    return _normalize_deepgemm_mhc_split(split_k)
...
def mhc_pre_gemm_sqrsum_tilelang(
    ...
    fn: T.Tensor((hc_mult3, hc_hidden_size), T.float32)  # type: ignore[no-redef, valid-type]
    out: T.Tensor((num_tokens, hc_mult3), T.float32)  # type: ignore[no-redef, valid-type]
    sqrsum: T.Tensor((num_tokens), T.float32)  # type: ignore[no-redef, valid-type]
    ...
    with T.Kernel(T.ceildiv(num_tokens, token_block)) as px:
        ...
        for pz in T.Pipelined(hc_hidden_size // hidden_block, num_stages=0):
            ...
            for i, j in T.Parallel(32, hidden_block):
                if i < hc_mult3:
                    fn_smem[i, j] = fn[i, pz * hidden_block + j]
                else:
                    fn_smem[i, j] = 0.0
...
def mhc_pre_gemm_sqrsum_splitk_kernel(
    ...
    def mhc_pre_gemm_sqrsum_splitk_stage_0(
        ...
        with T.Kernel(T.ceildiv(num_tokens, token_block), split_k, threads=threads) as (
            ...
            out_frag = T.alloc_fragment((token_block, 32), T.float32)
            sq_part4 = T.alloc_fragment((token_block, 8), T.float32)
            T.clear(out_frag)
            T.clear(sq_part4)
            ...
            for pz in T.Pipelined(split_size // hidden_block, num_stages=0):
                ...
                for i, j in T.Parallel(32, hidden_block):
                    if i < hc_mult3:
                        fn_smem[i, j] = fn[i, k_base + pz * hidden_block + j]
                    else:
                        fn_smem[i, j] = 0.0
...
def _tf32_hc_prenorm_gemm_tilelang(
    x: torch.Tensor,
    fn: torch.Tensor,
    out: torch.Tensor,
    sqrsum: torch.Tensor,
    ...
    num_tokens, hc_hidden_size = x.shape
    hc_mult3 = fn.shape[0]
    assert out.shape == (num_split, num_tokens, hc_mult3)
    assert sqrsum.shape == (num_split, num_tokens)

    if num_split != 1:
        out.zero_()
        sqrsum.zero_()
    out_view = out[:1]
    sqrsum_view = sqrsum[:1]

    if num_tokens <= 2048:
        if hc_hidden_size not in (16384, 28672):
            raise NotImplementedError(
                "TileLang MHC prenorm only supports "
                f"hc_hidden_size in (16384, 28672), got {hc_hidden_size}"
            )
        ...
        partial_out = out_view.new_empty(n_splits_pre, num_tokens, 32)
        partial_sqrsum = sqrsum_view.new_empty(n_splits_pre, num_tokens)
        kernel_0(x, fn, partial_out, partial_sqrsum)
        kernel_1(
            partial_out,
            partial_sqrsum,
            out_view.squeeze(0),
            sqrsum_view.squeeze(0),
        )
    else:
...
def mhc_fused_post_pre(
    ...
    if (
        ...
        if num_tokens <= 2048:
            use_tilelang_prenorm = hc_hidden_size in (16384, 28672)
    pre_n_splits = 1 if use_tilelang_prenorm else n_splits
    ...
    if num_tokens <= fma_token_threshold:
        ...
        if use_tilelang_prenorm:
            _tf32_hc_prenorm_gemm_tilelang(
