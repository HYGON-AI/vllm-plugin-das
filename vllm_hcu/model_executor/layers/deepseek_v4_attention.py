...
from vllm_hcu.v1.attention.ops.deepseek_v4_ops import (
...
from vllm_hcu.utils.multi_stream_utils import (
    execute_in_parallel,
...
class DeepseekV4MultiHeadLatentAttentionWrapper(PluggableLayer):
    ...
    def __init__(
        ...
        # Keep the output buffer head dimension aligned with the real local
        # head count. The ROCm FlashMLA path supports non-padded heads.
        if num_heads > 128:
            raise ValueError(
    ...
    def forward(
        ...
        if (
            ...
            and henvs.VLLM_HCU_DEEPSEEK_V4_ROCM_FAST_WOA
        ):
            torch.ops.hcu_ops.deepseek_v4_inv_rope(
            ...
            o = o.view(num_tokens, self.n_local_groups, hidden_dim)
            wo_a = self.wo_a.weight.view(
                self.n_local_groups,
                self.o_lora_rank,
                hidden_dim,
            )
            z = torch.einsum("tgd,grd->tgr", o, wo_a)
    ...
    def attn_gemm_parallel_execute(self, hidden_states) -> tuple[Any, ...]:
        ...
        # If aux streams are unavailable, execute_in_parallel runs serially.
        ...
        qr_kv, (kv_score, indexer_weights, indexer_kv_score) = execute_in_parallel(
            ...
            <= henvs.VLLM_HCU_DEEPSEEK_V4_MULTI_STREAM_GEMM_TOKEN_THRESHOLD,
        )
    ...
    def attention_impl(
        ...
        if self.indexer is not None:
            ...
            eager_overlap = (
                kv_score.shape[0]
                <= henvs.VLLM_HCU_DEEPSEEK_V4_MULTI_STREAM_COMPRESSOR_TOKEN_THRESHOLD
            )
            if eager_overlap:
        ...
        # Legacy q padding path, disabled because FlashMLA now supports
        # non-padded head counts.
