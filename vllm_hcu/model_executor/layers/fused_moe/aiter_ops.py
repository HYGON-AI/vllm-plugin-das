...
The shared implementation tracks the vLLM v0.28 target module. vLLM owns its
public interface, custom-op schemas, and communicator lifecycle; the narrow
HCU integration block preserves only the remaining device capability deltas.
"""
import ctypes
import functools
import os
...
def _rocm_aiter_rmsnorm_fused_add_dynamic_quant_impl(
    ...
    return _hcu_runtime.rmsnorm_add_dynamic_quant_impl(
...
def _rocm_aiter_rmsnorm_fused_dynamic_quant_impl(
    ...
    assert quant_dtype in [torch.int8, FP8_DTYPE]
    return _hcu_runtime.rmsnorm_dynamic_quant_impl(
...
class rocm_aiter_ops:
    ...
    # TODO: Consolidate under _LINEAR_ENABLED
    _FP4_GEMM_DYNAMIC_QUANT_ASM = getattr(
        envs, "VLLM_ROCM_USE_AITER_FP4_ASM_GEMM", False
    )
    ...
    def refresh_env_variables(cls):
        ...
        cls._FP4_GEMM_DYNAMIC_QUANT_ASM = getattr(
            envs, "VLLM_ROCM_USE_AITER_FP4_ASM_GEMM", False
        )
    ...
    def is_fp8bmm_enabled(cls) -> bool:
        return _hcu_runtime.is_triton_fp8_bmm_enabled(
            cls._AITER_ENABLED,
            cls._FP8BMM_ENABLED,
        )
    ...
    def is_shuffled_per_token_w8a8_gemm_tuned(
        ...
        csv_path = _hcu_runtime.get_w8a8_tuned_config_path(
...
# ---------------------------------------------------------------------------
# HCU v0.28 integration.  Keep this immediately before the sole registration
...
_vllm_hcu_original_is_aiter_found_and_supported = is_aiter_found_and_supported
_vllm_hcu_original_rocm_aiter_fused_moe_impl = _rocm_aiter_fused_moe_impl
_vllm_hcu_original_rocm_aiter_topk_softmax_impl = _rocm_aiter_topk_softmax_impl
_vllm_hcu_original_get_aiter_activation_type = (
    rocm_aiter_ops.get_aiter_activation_type
)


def is_aiter_found_and_supported() -> bool:
    return _hcu_runtime.is_aiter_found_and_supported(
        _vllm_hcu_original_is_aiter_found_and_supported,
        current_platform,
        IS_AITER_FOUND,
    )
...
def _rocm_aiter_fused_moe_impl(
    ...
    return _hcu_runtime.fused_moe_impl(
        _vllm_hcu_original_rocm_aiter_fused_moe_impl,
        hidden_states,
        w1,
        w2,
        topk_weight,
...
def _rocm_aiter_topk_softmax_impl(
    ...
    return _hcu_runtime.topk_softmax_impl(
        topk_weights,
        topk_indices,
        token_expert_indices,
        gating_output,
...
def _hcu_get_aiter_activation_type(activation_str: str):
    return _hcu_runtime.get_aiter_activation_type(
        _vllm_hcu_original_get_aiter_activation_type,
        activation_str,
    )
...
_vllm_hcu_aiter_ops_replacement = True
_HCU_REGISTER_OPS_CALLS = 0
_HCU_REGISTER_OPS_CALLS += 1
