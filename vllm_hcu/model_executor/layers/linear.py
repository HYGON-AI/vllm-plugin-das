...
def adjust_scalar_to_fused_array(
    ...
    if henvs.VLLM_USE_NN:
        return param_data[shard_id], loaded_weight.t()
    else:
...
class UnquantizedLinearMethod(LinearMethodBase):
    ...
    def apply(
        ...
        # if henvs.VLLM_USE_NN and x.shape[-1] == layer.weight.shape[0]:
...
class ReplicatedLinear(LinearBase):
    ...
    def weight_loader(self, param: Parameter, loaded_weight: torch.Tensor):
        ...
        if henvs.VLLM_USE_NN and not self.is_quantization:
...
class ColumnParallelLinear(LinearBase):
    ...
    def weight_loader(self, param: Parameter, loaded_weight: torch.Tensor):
        ...
        if output_dim is not None and not is_sharded_weight:
            if not henvs.VLLM_USE_NN or len(param_data.shape)==1 or self.is_quantization:
...
class DCPGroupColumnParallelLinear(ColumnParallelLinear):
    """Column-parallel linear sharded across DCP groups.

    This is the v0.28 framework contract implemented on top of the HCU linear
    adapter. Each rank in a DCP group owns the same group-wide head shard;
    prefill can recover the local TP view with :meth:`_local_view`.
    """
...
class MergedColumnParallelLinear(ColumnParallelLinear):
    ...
    def weight_loader(
        ...
        if output_dim is not None:
            ...
            if not henvs.VLLM_USE_NN or self.is_quantization or (henvs.VLLM_USE_NN and param_data.dim()==1):
...
class RowParallelLinear(LinearBase):
    ...
    def weight_loader(self, param: Parameter, loaded_weight: torch.Tensor):
        ...
        if input_dim is not None and not is_sharded_weight:
            if not henvs.VLLM_USE_NN or self.is_quantization:
