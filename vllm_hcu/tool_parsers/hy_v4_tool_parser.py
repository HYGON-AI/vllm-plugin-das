...
class HYV4ToolExtractor:
    ...
    def __init__(self, vocab: dict[str, int], token_suffix: str, strict: bool):
        ...
        self._buffer = ""
        self._stream_prefix_emitted_len = 0
        self._stream_suffix_emitted_len = 0
        self._stream_names_emitted: set[int] = set()
        self._stream_completed_calls: set[int] = set()
    ...
    def extract_tool_calls(
        ...
        if self.tool_calls_start_token not in model_output:
            ...
            try:
                ...
                e_index = model_output.find(
                    self.tool_calls_end_token,
                    s_index + len(self.tool_calls_start_token),
                )
                if s_index != -1 and e_index != -1:
                    content = (
                        model_output[:s_index]
                        + model_output[e_index + len(self.tool_calls_end_token) :]
                    )
                else:
    ...
    def extract_tool_calls_streaming(
        ...
        if marker_pos == -1:
            ...
            content = current_text[self._stream_prefix_emitted_len : safe_end]
            self._stream_prefix_emitted_len = safe_end
            if not content:
                return None
            return StreamDelta(content=content, tool_calls=[])

        prefix = current_text[:marker_pos]
        prefix_delta = prefix[self._stream_prefix_emitted_len :]
        self._stream_prefix_emitted_len = len(prefix)

        block_body_start = marker_pos + len(marker)
        block_end = current_text.find(self.tool_calls_end_token, block_body_start)
        block_body_end = block_end if block_end != -1 else len(current_text)
        block_body = current_text[block_body_start:block_body_end]

        tool_deltas: list[StreamToolCall] = []
        scan_from = 0
        call_index = 0
        while True:
            call_start = block_body.find(self.tool_call_start_token, scan_from)
            if call_start == -1:
                break
            body_start = call_start + len(self.tool_call_start_token)
            call_end = block_body.find(self.tool_call_end_token, body_start)
            complete = call_end != -1
            call_body = block_body[body_start : call_end if complete else None]

            arg_start = call_body.find(self.arg_key_start_token)
            if arg_start == -1 and not complete:
                break
            name_end = arg_start if arg_start != -1 else len(call_body)
            tool_name = call_body[:name_end].strip()
            if not tool_name:
                break

            name_new = call_index not in self._stream_names_emitted
            if name_new:
                self._stream_names_emitted.add(call_index)
                self.current_tool_id = max(self.current_tool_id, call_index)

            snapshot = self._stream_argument_snapshot(
                call_body,
                tool_name,
                complete,
                tools,
            )
            while len(self.streamed_args_for_tool) <= call_index:
                self.streamed_args_for_tool.append("")
            already_streamed = self.streamed_args_for_tool[call_index]
            if snapshot.startswith(already_streamed):
                argument_delta = snapshot[len(already_streamed) :]
            else:
                logger.warning(
                    "HYV4 streaming argument snapshot lost prefix stability "
                    "for tool index %d; withholding the inconsistent delta.",
                    call_index,
                )
                argument_delta = ""
            self.streamed_args_for_tool[call_index] = snapshot

            if complete and call_index not in self._stream_completed_calls:
                self._stream_completed_calls.add(call_index)
                try:
                    final_args = json.loads(snapshot)
            ...
            if name_new or argument_delta:
                tool_deltas.append(
                    StreamToolCall(
                        index=call_index,
                        new=name_new,
                        name=tool_name if name_new else None,
                        arguments=argument_delta or None,
                    )
                )
            ...
            if not complete:
                break
            scan_from = call_end + len(self.tool_call_end_token)

        suffix_delta = ""
        if block_end != -1:
            suffix = current_text[block_end + len(self.tool_calls_end_token) :]
            suffix_delta = suffix[self._stream_suffix_emitted_len :]
            self._stream_suffix_emitted_len = len(suffix)

        content = prefix_delta + suffix_delta
    ...
    def _stream_argument_snapshot(
        ...
        """Return the stable JSON prefix represented by a cumulative call."""
        arg_start = call_body.find(self.arg_key_start_token)
        args_text = call_body[arg_start:] if arg_start != -1 else ""
        completed_args: dict[str, Any] = {}
        last_closed_end = 0
        for match in self.func_args_regex.finditer(args_text):
            key, value = match.groups()
            completed_args[key.strip()] = _parse_value(
                value,
                tool_name,
                key.strip(),
                tools,
        ...
        if complete:
            return json.dumps(completed_args, ensure_ascii=False)
        ...
        key_start = tail.find(self.arg_key_start_token)
        if key_start != -1:
            key_end = tail.find(
                self.arg_key_end_token,
                key_start + len(self.arg_key_start_token),
            )
            if key_end != -1:
                key = tail[
                    key_start + len(self.arg_key_start_token) : key_end
                ].strip()
                value_start = tail.find(self.arg_value_start_token, key_end)
                if value_start != -1 and _is_only_string_type(
                    tool_name, key, tools
                ):
                    raw_value = tail[value_start + len(self.arg_value_start_token) :]
                    safe_end = len(raw_value) - partial_tag_overlap(
                        raw_value, self.arg_value_end_token
                    )
                    escaped = json.dumps(
                        raw_value[:safe_end], ensure_ascii=False
                    )[1:-1]
                    snapshot_parts.append(
                        f'{json.dumps(key, ensure_ascii=False)}: "{escaped}'
                    )
...
def _tools_to_plain(tools) -> list[ToolSchema] | None:
    ...
    for tool in tools:
        ...
        if function is None:
            if getattr(tool, "type", None) != "function" or not hasattr(
                tool, "name"
            ):
                continue
            function = tool
...
def _build_hyv4_structural_tag(
    tools,
    tool_choice,
    token_suffix: str,
):
    """Build the HYV4 grammar missing from vLLM 0.25.1/xgrammar."""
    ...
    from xgrammar.testing import _glm_xml_tool_calling_to_ebnf

    plain_tools = _tools_to_plain(tools)
    if not plain_tools or tool_choice == "none":
        return None

    named_tool: str | None = None
    if isinstance(tool_choice, ChatCompletionNamedToolChoiceParam):
        named_tool = tool_choice.function.name
    elif isinstance(tool_choice, ToolChoiceFunction):
        named_tool = tool_choice.name

    if named_tool is not None:
        plain_tools = [tool for tool in plain_tools if tool["name"] == named_tool]
        if not plain_tools:
            return None

    tool_calls_start = f"<tool_calls{token_suffix}>"
    tool_calls_end = f"</tool_calls{token_suffix}>"
    tool_call_start = f"<tool_call{token_suffix}>"
    tool_call_end = f"</tool_call{token_suffix}>"
    arg_key_start = f"<arg_key{token_suffix}>"
    arg_key_end = f"</arg_key{token_suffix}>"
    arg_value_start = f"<arg_value{token_suffix}>"
    ...
    for tool in plain_tools:
        ...
        grammar = _glm_xml_tool_calling_to_ebnf(
            parameters,
            # xgrammar's any-order mode intentionally does not enforce which
            # required keys are present. Keep canonical schema order when
            # required keys exist; optional-only tools can safely use any
            # argument order.
            any_order=not bool(parameters.get("required")),
        )
        for plain, suffixed in (
            ("<arg_key>", arg_key_start),
            ("</arg_key>", arg_key_end),
            ("<arg_value>", arg_value_start),
            ("</arg_value>", arg_value_end),
        ):
            grammar = grammar.replace(plain, suffixed)
        call_tags.append(
            TagFormat(
                begin=f"{tool_call_start}{tool['name']}",
                content=GrammarFormat(grammar=grammar),
                end=tool_call_end,
            )
        )

    tool_block = TagFormat(
        begin=tool_calls_start,
        content=TagsWithSeparatorFormat(
            tags=call_tags,
            separator="",
            at_least_one=True,
            stop_after_first=named_tool is not None,
        ),
        end=tool_calls_end,
    )
    if tool_choice in (None, "auto"):
        output_format = TriggeredTagsFormat(
            triggers=[tool_calls_start],
            tags=[tool_block],
        )
    else:
        output_format = tool_block
...
class HYV4ToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: TokenizerLike, tools=None):
        ...
        # Without structural-tag guidance, retain vLLM's JSON implementation
        # for required/named choices instead of asking this native-tag parser
        # to consume JSON output.
        self.supports_required_and_named = (
            not envs.VLLM_ENFORCE_STRICT_TOOL_CALLING
        )
        ...
        *,
        ...
        try:
            return _build_hyv4_structural_tag(
                request.tools,
                request.tool_choice,
                self._extractor.token_suffix,
            )
        except Exception:
            # Make the advertised fallback real: the unified serving parser
            # reads this flag after request adjustment and will use its JSON
            # required/named path when no structural tag could be built.
            self.supports_required_and_named = True
            logger.warning(
                "HYV4ToolParser failed to build a structural tag; falling back "
                "to vLLM's JSON guided-decoding path for required/named calls.",
                exc_info=True,
            )
            return None
    ...
    def adjust_request(
        ...
        """Keep required/named calls in HYV4's native tagged format.

        vLLM's base implementation installs JSON guided decoding for these
        tool choices. HYV4 instead emits ``<tool_calls>`` structural tokens,
        so JSON guidance produces output this parser cannot consume.
        """
        if request.tools and not self.supports_required_and_named:
